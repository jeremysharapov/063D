...
void add_sphere( struct matrix * edges, 
  ...
  for (i = 0; i < sphere -> lastcol; i++){
    add_edge(edges, sphere -> m[0][i], sphere -> m[1][i], sphere -> m[2][i], sphere -> m[0][i] + 1, sphere -> m[1][i], sphere -> m[2][i]);
  }
  free_matrix(sphere);
}
...
struct matrix * generate_sphere(double cx, double cy, double cz,
  ...
  int i, j;
  for (i = 0; i <= step; i++){
    phi = (double) 2 * M_PI * i / step;
    for (j = 0; j <= step; j++){
      theta = (double) M_PI * j / step;
      add_point(sphere, r * cos(theta) + cx, r * sin(theta) * cos(phi) + cy, r * sin(theta) * sin(phi) + cz);
    }
  }
  return sphere;
}
...
void add_torus( struct matrix * edges, 
  ...
  for (i = 0; i < torus -> lastcol; i++){
    add_edge(edges, torus -> m[0][i], torus -> m[1][i], torus -> m[2][i], torus -> m[0][i] + 1, torus -> m[1][i], torus -> m[2][i]);
  }
  free_matrix(torus);
}
...
struct matrix * generate_torus( double cx, double cy, double cz,
  ...
  for (i = 0; i <= step; i++){
    phi = (double) 2 * M_PI * i / step;
    for (j = 0; j <= step; j++){
      theta = (double) 2 * M_PI * j / step;
      add_point(torus, cos(phi) * (r1 * cos(theta) + r2) + cx, r1 * sin(theta) + cy, -1 * sin(phi) * (r1 * cos(theta) + r2) + cz);
    }
  }
  return torus;
}
